...
- `cj init` pre-registers a tmux session with a topic; on `SessionStart` the hook migrates the topic onto the real session row.
...
- 39 unit + integration tests covering pure helpers, transcript parsing, and the DB layer against in-memory SQLite.
- GitHub Actions CI matrix on Ubuntu + macOS running `cargo fmt --check`, `cargo clippy -D warnings`, and `cargo test`.

[Unreleased]: https://github.com/your-org/claude-jam/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/your-org/claude-jam/releases/tag/v0.1.0
